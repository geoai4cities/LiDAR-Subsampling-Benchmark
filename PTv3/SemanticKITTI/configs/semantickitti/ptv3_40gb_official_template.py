"""
PTv3 Configuration for SemanticKITTI Dataset - 40GB GPU (Official Outdoor Settings)
...
experiment = dict(
    name="ptv3_semantickitti_{METHOD}_loss{LOSS}_seed{SEED}_40gb",
...
# Training settings - 40GB GPU optimized
batch_size = 6  # Smaller batch for 40GB GPU
batch_size_val = 1  # MUST be 1 for correct inverse mapping in SemSegEvaluator
batch_size_test = 1
num_worker = 12
num_worker_test = 1
gradient_accumulation_steps = 8  # Effective batch = 48 (6 * 8)
mix_prob = 0.8
empty_cache = True  # Enable for memory-constrained GPU
...
wandb = dict(
    ...
    name="ptv3_semantickitti_{METHOD}_loss{LOSS}_seed{SEED}_40gb",
    entity=None,
    tags=["semantickitti", "a100-40gb", "ptv3", "{METHOD}", "loss{LOSS}"],
)
