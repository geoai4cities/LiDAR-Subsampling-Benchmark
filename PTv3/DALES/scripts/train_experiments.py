#!/usr/bin/env python3
"""
PTv3 Training Script for DALES Dataset

Manages training experiments for DALES with all subsampling methods.
...
DATASET_NAME = "DALES"
CONFIG_DIR = DATASET_DIR / "configs" / "dales" / "generated"
...
def get_config_path(method: str, loss: int, seed: int) -> Path:
    ...
    config_name = f"ptv3_dales_{method}_loss{loss}_seed{seed}.py"
...
def list_experiments(experiments):
    ...
    print(f"\n{'='*70}")
    print(f"DALES Experiments ({len(experiments)} total)")
    print(f"{'='*70}")
...
def run_experiment(method: str, loss: int, seed: int, gpu_id: int = 0, dry_run: bool = False):
    ...
    exp_name = f"ptv3_dales_{method}_loss{loss}_seed{seed}"
    ...
    cmd = [
        ...
        "--num-gpus", "1",
        "--options",
        f"save_path={BENCHMARK_ROOT}/experiments/checkpoints/ptv3/DALES/{exp_name}"
    ]
