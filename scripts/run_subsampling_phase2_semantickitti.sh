#!/bin/bash
################################################################################
# Phase 2: SemanticKITTI - GPU Accelerated Subsampling (IDIS / FPS)
#
...
################################################################################
#
#   ./run_subsampling_phase2_semantickitti.sh [OPTIONS]
#
#   Options:
...
#     -R RADIUS         IDIS radius in meters (default: 10, options: 5, 10, 15, 20)
#
################################################################################
# Examples
################################################################################
...
#   ./run_subsampling_phase2_semantickitti.sh --method IDIS --loss 50
#   ./run_subsampling_phase2_semantickitti.sh --method IDIS -R 5 --loss 50
#   ./run_subsampling_phase2_semantickitti.sh --method IDIS -R 15 --loss 50
#
#   # FPS - requires seed
#   ./run_subsampling_phase2_semantickitti.sh --method FPS --loss 50 --seed 1
#
#   # Generate all loss levels
#   ./run_subsampling_phase2_semantickitti.sh --method IDIS
#   ./run_subsampling_phase2_semantickitti.sh --method FPS --seed 1
#
#   # Default: IDIS with R=10m, all loss levels
#   ./run_subsampling_phase2_semantickitti.sh
#
################################################################################
# Method Properties
################################################################################
...
#   - No seed required
#   - Radius parameter (-R): 5m, 10m (default), 15m, 20m
#   - Output: IDIS_loss{XX}/ or IDIS_R{5,15,20}_loss{XX}/
#
...
#
# Resume capability:
#   - Script automatically skips already-processed scans
#   - Safe to re-run after interruption or partial completion
#
# Performance (GPU - NVIDIA A100/H100):
#   - IDIS: ~0.3s per scan (vs ~30-60s CPU) = ~100-200x speedup
#   - FPS:  ~1.6s per scan (vs ~60-120s CPU) = ~40-70x speedup
#
################################################################################
...
print_usage() {
    ...
    echo ""
    echo "Phase 2: GPU-accelerated subsampling for SemanticKITTI (IDIS / FPS)"
    echo ""
    echo "Options:"
    ...
    echo "  -R RADIUS         IDIS radius in meters (5, 10, 15, 20)"
    ...
    echo "  IDIS: data/SemanticKITTI/subsampled/IDIS_loss{XX}/"
    echo "  IDIS (R!=10): data/SemanticKITTI/subsampled/IDIS_R{5,15,20}_loss{XX}/"
    echo "  FPS:  data/SemanticKITTI/subsampled/FPS_loss{XX}_seed{N}/"
}
...
LOG_FILE="$LOG_DIR/phase2_semantickitti_${TIMESTAMP}.log"
...
echo ""
echo "================================================================"
echo "  Phase 2: SemanticKITTI - GPU Accelerated Subsampling"
echo "================================================================"
echo ""
...
if [[ "$METHOD" == "IDIS" ]]; then
    # IDIS - deterministic, no seed needed
    PYTHONUNBUFFERED=1 python scripts/preprocessing/generate_subsampled_gpu.py \
        --method IDIS \
        --radius ${RADIUS}.0 \
        $RADIUS_SUFFIX_ARG \
...
# ============================================================================
echo "================================================================"
echo "  Phase 2 Complete - SemanticKITTI [GPU]"
echo "================================================================"
echo ""
...
if [[ "$METHOD" == "IDIS" ]]; then
    echo "  data/SemanticKITTI/subsampled/IDIS${RADIUS_SUFFIX}_loss{XX}/"
else
    echo "  data/SemanticKITTI/subsampled/FPS_loss{XX}_seed${SEED}/"
fi
echo ""
echo "GPU Acceleration Summary:"
if [[ "$METHOD" == "IDIS" ]]; then
    echo "  - IDIS: ~0.3s per scan (vs ~30-60s CPU) = ~100-200x speedup"
else
    echo "  - FPS: ~1.6s per scan (vs ~60-120s CPU) = ~40-70x speedup"
fi
echo ""
echo "Next step: Start training experiments"
echo "  cd PTv3/SemanticKITTI/scripts"
echo "  ./train_semantickitti_140gb.sh --method $METHOD --loss 50 start"
echo ""
echo "Log file: $LOG_FILE"
echo ""
